...
def postprocess_per_image(
    ...
    pred_depth = pred_depth[
        pad[0] : pred_depth.shape[0] - pad[1], pad[2] : pred_depth.shape[1] - pad[3]
    ]
    pred_depth = torch.nn.functional.interpolate(
        pred_depth[None, None, :, :],
        [rgb_origin.shape[0], rgb_origin.shape[1]],
        mode="bilinear",
    ...
    if save_pred_dir is not None:
        np.savez_compressed(
            osp.join(save_pred_dir, an["filename"][:-4] + ".npz"),
    ...
    #                 osp.join(
    #                     save_pcd_dir, an["folder"], an["filename"][:-4] + fstr + ".ply"
    #                 ),
    #             )
    ...
    if normal_out is not None:
        ...
        pred_normal = torch.nn.functional.interpolate(
            pred_normal[None, :],
            size=[rgb_origin.shape[0], rgb_origin.shape[1]],
            mode="bilinear",
            align_corners=True,
        ).squeeze()
        ...
        if save_pred_dir is not None:
            np.savez_compressed(
                osp.join(save_pred_dir, an["filename"][:-4] + "_normal.npz"),
                pred_normal.detach().cpu().numpy(),
            )
